...
Since these are used in abundance for both procedures, their constrains are placed in the Lagrangian term. Other constraints (such specified volume, temperature, internal energy, and entropy) are given their rows in the Newton solve without addition into the Lagrangian. 

\vspace{5mm} 

Equation [$\ref{elementalconstraint}$] says that the number of moles of element $i$ must remain constant during the minimization process, as atoms can not be created or destroyed. $b_i^\circ$ is the number of moles of element $i$ before minimizing, and $a_{i,j}$ is the stoichiometric coefficient, e.g. how many atoms of element $i$ are in species $j$. 

Equation [$\ref{chargeconstraint}$] says that the charge of the system must remain neutral, i.e., if cations are formed, than there must be an equal number of free lectrons in the gas to balance it out.
...
Here, $f(x)$ is the function that is being minimized. In the case of Gibbs and Helmholtz energy minimization, $f$ takes the form of $\partial \mathcal{L} / \partial x$, with $x$ being non-linear variables used to guide the convergence of the system. The non-linear correction variables are of the form $\Delta \ln \mathcal{N}_j$, $\Delta \ln\mathcal{N}$, $\Delta \ln T$, and $\pi_{i,q} = \Delta \pi_{i,q}$. For the last correction variable, it is argued that setting $\pi$ to 0 at the beginning of each update does not influence the solution for the non-reduced equations, so the $\Delta$ is dropped for that term. This will influence the form of the update equations, which will be talked about in due time.
...
\begin{align}
    f_4 &= \frac{h' - h_0'}{\hat{R}T} = \frac{\sum_{j=1}^\text{NS} \mathcal{N}_j H_j^\circ - h_0'}{\hat{R}T}
    \\ f_5 &= \frac{s' - s_0'}{\hat{R}} = \frac{\sum_{j=1}^\text{NS} \mathcal{N}_j S_j - s_0'}{\hat{R}}
\end{align}
...
We now need to take the partial derivatives of our functions wrt our non-linear variables as well as each $\pi$, and then multiple it by the correction variables. We start with equation \ref{f11}:
...
\begin{align}
    ...
    \\ \frac{\partial f_3}{\partial [\ln \mathcal{N}]} \Delta\ln \mathcal{N}_j &= 0
...
\begin{align}
    \frac{\partial f_4}{\partial [\ln{\mathcal{N}_j}]} \Delta \ln \mathcal{N}_j &=  
    \\ \frac{\partial f_4}{\partial [\ln \mathcal{N}]} \Delta\ln \mathcal{N}_j &= 0
    \\ \frac{\partial f_4}{\partial [\ln T]} &= 
...
\begin{align}
    \frac{\partial f_5}{\partial [\ln{\mathcal{N}_j}]} \Delta \ln \mathcal{N}_j &=  
    \\ \frac{\partial f_5}{\partial [\ln \mathcal{N}]} \Delta\ln \mathcal{N}_j &= 0
    \\ \frac{\partial f_5}{\partial [\ln T]} &= 
...
and $R' = \hat{R} \cdot 10^{-5}$ is the same as dividing $\hat{R}$ by the reference pressure of 1 bar. The contraints available for the Helmholtz energy are:
...
Finally, we look towards the constraints. For internal energy, we have:
...
\begin{equation}
    \textcolor{red}{\Delta \ln \mathcal{N}_j =  \sum_{i=1}^\text{NE}a_{i,j} -\frac{\mu_j}{\hat{R}T} \pi_i} \textcolor{blue}{+ q_j \pi_q}\textcolor{violet}{ + \frac{U_j^\circ}{\hat{R}T} \Delta \ln T }
    \label{redH0}
\end{equation}
